...
- **TS-5: File attachments for Task Schedule** - design at `TS-5_PLAN.md` at the repo root. Lets Blake attach drawings (PDFs primarily) to a task schedule so his shop crew can pull them from the share link instead of digging through email. Show-wide Drawings sidebar + optional per-task attach via paperclip icon. Storage in the existing R2 bucket. ~6-8 hours focused. When starting: *"Read TS-5_PLAN.md and start implementing."*
